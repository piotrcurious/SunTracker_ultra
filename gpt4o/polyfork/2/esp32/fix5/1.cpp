...
public:
    ...
    ModularRing divide_by_const(int32_t divisor) const {
        return ModularRing((value * SCALE) / divisor, modulus);
    }
...
void Calculate_Sun_Position(int hour, int minute, int second, int day, int month, int year) {
    ...
    int32_t elevation = asin((Lat * DEG_TO_RAD_SCALED) * Decl + taylor_cos(Lat * DEG_TO_RAD_SCALED) * taylor_cos(Decl) * taylor_cos(HrAngle.toDegrees() * DEG_TO_RAD_SCALED));
    int32_t azimuth = normalize_angle(atan2(taylor_sin(HrAngle.toDegrees() * DEG_TO_RAD_SCALED), taylor_cos(HrAngle.toDegrees() * DEG_TO_RAD_SCALED)) + 180 * SCALE);
