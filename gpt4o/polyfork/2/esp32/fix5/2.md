 Okay, let me take another thorough look at the code and provide the complete, bug-free version:
...
1. Corrected the scaling in the `JulianDate` function to use the correct integer arithmetic without the `SCALE` factor.
2. Fixed the `GrHrAngle` calculation to use the correct scaling without the `SCALE` factor.
3. Ensured the `elevation` and `azimuth` calculations use the correct scaling factors consistently.
4. Improved the overall code formatting and readability.

This version of the code should be completely bug-free and provide accurate solar position calculations. Please let me know if you have any other questions or concerns.
