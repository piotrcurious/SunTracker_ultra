import os
import json
from sympy import symbols, Poly, GF, cos, pi
...
# LFSR utilities
def is_irreducible(poly, field_modulus):
    """
    Check if a polynomial is irreducible over a given field.

    :param poly: The polynomial to check.
    :param field_modulus: The modulus of the field (prime number).
    :return: True if the polynomial is irreducible, False otherwise.
    """
    # Construct the polynomial in the Galois Field
    poly_over_gf = Poly(poly, domain=GF(field_modulus))

    # Factorize and check if it is irreducible
    factors = poly_over_gf.factor_list()[1]
...
def search_lfsr_pairs(field_modulus: int, lfsr_length: int, n_points: int) -> None:
    ...
    sine_values = [int((15 + 15 * cos(2 * pi * i / n_points)).round()) for i in range(n_points)]
