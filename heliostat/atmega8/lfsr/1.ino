...
// Cosine table is derived from the sine table using phase shift (cos(x) = sin(90° - x))
...
float fastSin(int angle) {
  ...
  int index = angle / 5;  // Integer division for 5-degree intervals
  int nextIndex = (index + 1) % TABLE_SIZE;

  // Interpolation between table entries
  float weight = (angle % 5) / 5.0;
  int16_t value1 = pgm_read_word(&sinTable[index]);
  int16_t value2 = pgm_read_word(&sinTable[nextIndex]);
  return (value1 + weight * (value2 - value1)) / 1000.0;
}
